...
func init() {
	_ = filepath.Walk("testdata", func(path string, info os.FileInfo, err error) error {
		if filepath.Ext(path) == ".xml" && !strings.Contains(filepath.Base(path), ".fmt.xml") {
			st := stage{}
			st.key = strings.Replace(filepath.Base(path), ".xml", "", 1)
...
func assertStr(tb testing.TB, vec *Vector, path, expect string) {
	...
	if node = vec.Dot(path); node.Type() != vector.TypeStr {
		tb.Error("node type mismatch, need", vector.TypeStr, "got", node.Type())
		return
	}
